...
from .deformable_transformer_san_inDecoder import build_deforamble_transformer
...
class SgMg(nn.Module):
    ...
    def __init__(self, args, backbone, transformer, num_classes, num_queries, num_feature_levels,
        ...
        #todo init module
        self.text_language_fuse = LanguageVisionFusionModule(d_model=hidden_dim, nhead=8)
        self.get_clip_features = get_clip_features("ViT-B/16")
        self.clip_text_linear = nn.Linear(768, 256)

        self.hs_linear = nn.Linear(self.num_queries*2, self.num_queries)
        self.reference_linear = nn.Linear(self.num_queries*2, self.num_queries)
        self.references_linear = nn.Linear(self.num_queries*2, self.num_queries)
    ...
    def forward(self, samples: NestedTensor, captions, targets):
        ...
        for l, (feat, pos_l) in enumerate(zip(features[-multi_scale_level:], visual_pos[-multi_scale_level:])):
            ...
            # # text_l = repeat(text_word_features, 'l b c -> h l b c', h=h)
            # # text_l = repeat(text_l, 'h l b c -> t h l b c', t=t)
            # # print(text_l.shape, 'text_l') #5 24 12 1 256
            ...
            # lan_l = self.text_language_fuse(tgt=text_word_features,
            #                                memory=src_l,
        ...
        # todo add clip text
        # clip_text = self.clip_text_linear(clip_features["9_cls_token"].squeeze(0))
        # clip_text = repeat(clip_text, 'bt c -> q bt c', q=self.num_queries)
        # clip_text = rearrange(clip_text, 'q (b t) c -> b t q c', b=b)
        # text_embed = text_embed+clip_text

        hs, memory, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, inter_samples = \
                                            self.transformer(srcs, text_embed, masks, poses, clip_features, query_embeds)
        ...
        # init_reference: [batch_size*time, num_queries_p er_frame, 2]
        # inter_references: [l, batch_size*time, num_queries_per_frame, 4]

        # print(hs.shape, 'hs') #4 5 10 256
        ...
        for lvl in range(hs.shape[0]):
            ...
            outputs_seg_masks_formatcher.append(rearrange(F.pixel_shuffle(outputs_seg_mask.flatten(0,1), 4).squeeze(1), '(b t q) h w -> b t q h w', b=b, t=t))
