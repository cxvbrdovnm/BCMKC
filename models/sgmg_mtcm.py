...
from .mtcm_module import ReferringTracker_RVOS
...
class SgMg(nn.Module):
    ...
    def __init__(self, args, backbone, transformer, num_classes, num_queries, num_feature_levels,
        ...
        if num_feature_levels > 1:
            ...
            # print(num_backbone_outs, 'backbone_out')  #3
            input_proj_list = []
            input_fft_list = []
            input_fft_post_list = []
        ...
        # Build MSO
        # print(backbone.num_channels, 'backbone c') #[128, 256, 512, 1024]
        ...
        # todo
        self.tracker = ReferringTracker_RVOS(
    ...
    def forward(self, samples: NestedTensor, captions, targets):
        ...
        # for l, (feat, pos_l) in enumerate(zip(features, visual_pos)):
        #     src, mask = feat.decompose()
        #     print(src.shape)
        #     print(mask.shape)
        #     print(pos_l.shape)
            # torch.Size([5, 128, 56, 48])
            # torch.Size([5, 56, 48])
            # torch.Size([5, 256, 56, 48])

            # torch.Size([5, 256, 28, 24])
            # torch.Size([5, 28, 24])
            # torch.Size([5, 256, 28, 24])

            # torch.Size([5, 512, 14, 12])
            # torch.Size([5, 14, 12])
            # torch.Size([5, 256, 14, 12])

            # torch.Size([5, 1024, 7, 6])
            # torch.Size([5, 7, 6])
            # torch.Size([5, 256, 7, 6])
        ...
        #todo tracker
        vita_frame_embds = hs.view(4, b, t, self.num_queries, -1)

        tar_h, tar_w = memory[0].shape[-2:]
        memory_fusion = sum(
            [F.interpolate(x, size=(tar_h, tar_w), mode="bicubic", align_corners=False) for x in memory])  # bt, c, h, w
        mask_features = rearrange(memory_fusion, '(b t) c h w -> b t c h w', b=b, t=t)

        tracker_logits, tracker_masks = self.tracker(vita_frame_embds, mask_features, text_sentence_features.repeat(self.num_queries, 4, 1), None, resume=False)
        # print(tracker_logits.shape, "tracker_logits") #20 5 2
        # print(tracker_masks.shape, "tracker_masks") #20 5 16 28

        tracker_logits = rearrange(tracker_logits, "(l b q) t c -> l b t q c", l=4, b=b) #c=2: background+object
        tracker_logits = tracker_logits[:, :, :, :, -1].unsqueeze(4)
        tracker_masks = F.interpolate(tracker_masks, scale_factor=4, mode='nearest')
        tracker_masks = rearrange(tracker_masks, "(l b q) t h w -> l b t q h w", l=4, b=b)
        ...
        out['pred_logits'] = outputs_class[-1] + tracker_logits[-1]
        # print(outputs_class[-1].shape, "outputs_class[-1]") # 1 5 5 1
        ...
        # print(mask_features.shape, 'mask feat') #1 5 256 28 24
        ...
        for lvl in range(hs.shape[0]):
            ...
            # print(outputs_seg_mask.shape, "outputs_seg_mask") #1 25 16 28 28
        ...
        # for o in outputs_seg_masks_formatcher:
        #     print(o.shape) #1 5 5 112 112

        if self.training:
            # *** Perform HUNGARIAN matching before decoding ***
            with torch.no_grad():
                out['pred_masks'] = outputs_seg_masks_formatcher[-1] + tracker_masks[-1]
                # print(out['pred_masks'].shape) #1 5 5 112 96
                indices = self.matcher(out, targets, b)
                out["main_matcher_index"] = indices
                select_idxs = _get_src_permutation_idx(indices)

                if self.aux_loss:
                    new_formathcer = [a + b for a, b in zip(outputs_seg_masks_formatcher, tracker_masks)]
                    out['aux_outputs'] = self._set_aux_loss(outputs_class+tracker_logits, outputs_coord, new_formathcer)
                    aux_indices = []
            ...
            if self.aux_loss:
                ...
                # todo
                # outputs_seg_masks_formatcher = [x.transpose(1, 2)[y] for x, y in zip(outputs_seg_masks_formatcher+tracker_masks, select_idxs)]  # b t h w
                outputs_seg_masks_formatcher = [x.transpose(1, 2)[y] for x, y in zip(new_formathcer, select_idxs)]  # b t h w

                out['pred_masks_low'] = outputs_seg_masks_formatcher[-1]
                out['aux_outputs'] = self._set_aux_loss_comprehensive(outputs_class+tracker_logits, outputs_coord, outputs_seg_masks, outputs_seg_masks_formatcher)
