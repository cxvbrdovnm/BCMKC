...
from .rpe_attention_function_gate import rpe_multi_head_attention_forward
from . import irpe
...
class RPEMultiheadAttention(nn.Module):
    ...
    def get_rel_indices_hw(self, hw):
        # print(hw) #7,7
        h, w = hw
        rel_indices = torch.zeros(1, h * w, h * w, 3)
        ind_x = torch.arange(w).view(1, -1) - torch.arange(w).view(-1, 1)
        ind_y = torch.arange(h).view(1, -1) - torch.arange(h).view(-1, 1)

        indx = ind_x.repeat(h, h)
        indy = ind_y.repeat_interleave(w, dim=0).repeat_interleave(w, dim=1)
        ...
        #device = self.rpe_q.device
        #self.rel_indices = rel_indices.cuda()
    ...
    Shapes for outputs:
        ...
        if not self._qkv_same_embed_dim:
            return rpe_multi_head_attention_forward(
                ...
                rel_indices=self.get_rel_indices_hw(hw),
