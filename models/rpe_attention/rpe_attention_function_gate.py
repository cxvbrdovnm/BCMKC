...
def rpe_multi_head_attention_forward(
    ...
    #todo add gate
    patch_score = attn_output_weights.view(bsz, num_heads, *attn_output_weights.shape[-2:]) #b head h w
    pos_score = rel_indices.expand(bsz, -1, -1, -1) #(1, h*w, h*w, 3)
    pos_score = pos_proj(pos_score).permute(0, 3, 1, 2) #b head hw hw
    pos_score = softmax(pos_score, dim=-1)

    gating = gating_param.view(1, -1, 1, 1) #1 head 1 1
    attn = (1. - torch.sigmoid(gating)) * patch_score + torch.sigmoid(gating) * pos_score
    attn /= attn.sum(dim=-1).unsqueeze(-1)
    attn = attn_drop(attn)

    attn_output_weights = attn.view(
        bsz*num_heads, *attn_output_weights.shape[-2:])
