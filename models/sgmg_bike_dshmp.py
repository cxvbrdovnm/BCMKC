...
from .segmentation import VisionLanguageFusionModule, LanguageVisionFusionModule
...
class SgMg(nn.Module):
    ...
    def __init__(self, args, backbone, transformer,
        ...
        # self.bike_clip = bike_model  # clip model
        # self.video_head = video_head  # S_V
        # self.classes = classes
        self.bike_clip, self.video_head, self.classes = bike(args)
        # b-32: 512, l-14: 768
        clip_dim = 512
        self.clip_visual_linear = nn.Linear(clip_dim, 256)
        self.clip_visual_text_linear = nn.Linear(clip_dim, 256)

        self.fusion_query = LanguageVisionFusionModule(d_model=hidden_dim, nhead=8)
        self.clip_cls_linear = nn.Linear(512, 256)

        self.nlp = spacy.load("en_core_web_sm")

        # Cross-modal Alignment Branch
        # Channel Modulation
        num_channels_reduced = hidden_dim // 8
        self.fc1 = nn.Linear(hidden_dim, num_channels_reduced, bias=True)
        self.fc2 = nn.Linear(num_channels_reduced, hidden_dim, bias=True)
        self.relu = nn.ReLU()
        self.sigmoid = nn.Sigmoid()

        # Context Pixel-Attention
        self.context_pixel_attn = PWAM(hidden_dim,  # both the visual input and for combining, num of channels
                                       hidden_dim,  # v_in
                                       256,  # l_in  # 768 for bert feature, 512 for clip
                                       hidden_dim,  # key
                                       hidden_dim,  # value
                                       num_heads=8,
                                       dropout=0)
        ...
        # Object-Position Alignment Block
        self.object_position_align_blk = OPAB(hidden_dim,
                                              hidden_dim,
                                              256,
                                              hidden_dim,
                                              hidden_dim,
    ...
    def forward(self, samples: NestedTensor, captions, targets):
        ...
        # print(text_word_masks.shape) #1 12

        #todo dshmp
        motion_map, static_map = self.proceesing_text_decoupling(captions[0])
        motion_map = motion_map.to(text_word_features.device)
        static_map = static_map.to(text_word_features.device)

        # motion_feat = torch.cat([text_sentence_features[motion_map.bool()], text_features], dim=0)
        # static_feat = torch.cat([text_sentence_features[static_map.bool()], text_features], dim=0)
        motion_feat = text_word_features[motion_map.bool()]
        static_feat = text_word_features[static_map.bool()]
        # print(motion_feat.shape) #3 1 256
        # print(static_feat.shape) #7 1 256
        motion_mask = text_word_masks.transpose(0, 1)[motion_map.bool()]
        static_mask = text_word_masks.transpose(0, 1)[static_map.bool()]
        # print(motion_mask.shape) #3 1
        # print(static_mask.shape) #7 1


        #todo get clip feature
        num_segments = t # 视频帧数
        ...
        #vit-B 224 vit-L 336
        img_size = 224
        images_resized = F.interpolate(images_clip, size=(img_size, img_size), mode='bilinear', align_corners=False)
        all_class = self.classes.to(images_resized.device)
        # print(all_class.shape, "all class")  # 89 77
        # print(all_class)

        image_embedding, cls_embedding, text_embedding, logit_scale = \
            self.bike_clip(images_resized, all_class, return_token=True)
        ...
        image_embedding_sv = image_embedding.clone().view(b_o, t_o, -1)
        images_fusion, S_V = self.video_head(image_embedding_sv, text_embedding, cls_embedding)
        ...
        if 'valid_indices' not in targets[0]:
            max_index = torch.argmax(S_V)
            # print(S_V.shape)
            # print(max_index)
            # print(images_fusion.shape)  # 1 512
            # 获取与最相关类的视频特征
            ...
            # for a2d
            # caculate sv feature for each valid_indices
            clip_visual_add = []
            for v_id in valid_indices:
                max_index = torch.argmax(S_V.index_select(0, v_id))
                # 获取与最相关类的视频特征
                # print(images_fusion.index_select(1, v_id).squeeze(1).shape) # 93 512
                clip_visual_text = self.clip_visual_text_linear(
                    images_fusion.index_select(1, v_id).squeeze(1)[max_index].float())
                # print(clip_visual_text.unsqueeze(0).shape) # 1 512
                # print(clip_visual.index_select(0, v_id).shape) # 1 512
                clip_visual_add.append((clip_visual.index_select(0, v_id) + clip_visual_text.unsqueeze(0)))

            clip_visual_add = torch.stack(clip_visual_add).squeeze(1)
            # print(clip_visual_add.shape, "clip add")


        # todo S_A
        # S_A = self.sentence_head(cls_embedding, text_sentence_features)
        # print(S_A.shape, 'S_A')  # 1 89
        # print(S_A)
        # sa_max_index = torch.argmax(S_A)
        # print(sa_max_index)

        # clip_cls_new = self.clip_cls_linear(cls_embedding * logit_scale)
        # text_sentence_features_new = self.fusion_query(text_sentence_features.unsqueeze(1), clip_cls_new.unsqueeze(1)).squeeze(1)
        # print(text_sentence_features)
        # print(text_sentence_features_new.shape) #1 256
        # print(text_sentence_features_new)
        # print(cls_embedding[max_index].shape) #512
        # print(text_sentence_features)
        # print(cls_embedding[max_index])
        if 'valid_indices' not in targets[0]:
            clip_cls_new = self.clip_cls_linear((cls_embedding[max_index]).unsqueeze(0)).unsqueeze(0)
            text_sentence_features_new = self.fusion_query(text_sentence_features.unsqueeze(1), clip_cls_new).squeeze(1)
        else:
            # for a2d
            # caculate sv feature for each valid_indices
            text_sentence_features_new = []
            for i, v_id in enumerate(valid_indices):
                max_index = torch.argmax(S_V.index_select(0, v_id))
                # print(max_index)
                clip_cls_new = self.clip_cls_linear((cls_embedding[max_index]).unsqueeze(0))
                # print(clip_cls_new.shape)
                # print(text_sentence_features[i].shape)
                text_sentence_features_new_vid = self.fusion_query(text_sentence_features[i].unsqueeze(0).unsqueeze(1),
                                                                   clip_cls_new).squeeze(1)
                # print(text_sentence_features_new_vid.shape)
                text_sentence_features_new.append(text_sentence_features_new_vid)
            text_sentence_features_new = torch.stack(text_sentence_features_new).squeeze(1)
            # print(text_sentence_features_new.shape)
        ...
        for l, (feat, pos_l) in enumerate(zip(features[-multi_scale_level:], visual_pos[-multi_scale_level:])):
            ...
            #todo add clip visual feature
            src_proj_l = src_proj_l + clip_visual_add.view(n, c, 1, 1).expand(n, c, h, w)
        ...
        if self.num_feature_levels > len(srcs):
            ...
            for l in range(_len_srcs, self.num_feature_levels):
                ...
                # todo add clip visual feature
                src = src + clip_visual_add.view(n, c, 1, 1).expand(n, c, h, w)
        ...
        #     print(srcs[l].shape)
        # print(h, w)
        # torch.Size([6, 256, 20, 28])
        # torch.Size([6, 256, 10, 14])
        # torch.Size([6, 256, 5, 7])
        # torch.Size([6, 256, 3, 4])
        # 3 4


        #todo
        static_feat = repeat(static_feat, "l b c -> (b t) c l", t=t)  # bt c l
        motion_feat = repeat(motion_feat, "l b c -> (b t) c l", t=t)
        # print(static_mask.shape) #7 1
        static_mask = repeat(static_mask, "l b -> (b t) l", t=t).unsqueeze(2)  # bt l 1
        motion_mask = repeat(motion_mask, "l b -> (b t) l", t=t).unsqueeze(2)
        # print(static_mask.shape) #5 7 1
        ...
        #     z = src.permute(0, 2, 1).mean(dim=2)
        #     fc_out_1 = self.relu(self.fc1(z))
        #     fc_out_2 = self.sigmoid(self.fc2(fc_out_1))
        #
        #
        #     # x_residual = self.context_pixel_attn(src_re, text_word_features, text_word_masks.unsqueeze(2).permute(1,0,2)) # l b c
        #     x_residual = src
        #     x_residul_gate = self.context_tanh_gate(x_residual) * x_residual
        #
        #     op_align = self.object_position_align_blk(src, H, W, static_feat, static_mask, motion_feat, motion_mask) #l b
        #     x_add = x_residul_gate + op_align
        #
        #     # Channel Modulation
        #     x_add = x_add * fc_out_2.unsqueeze(1)
        #     src_new = src + x_add
        #     # print(src_new.shape) #5 448 256
        #     src_new = rearrange(src_new, 'bt (h w) c -> bt c h w', h=H)
        #
        ...
        #todo new query
        text_embed = repeat(text_sentence_features_new, 'b c -> b t q c', t=t, q=self.num_queries)
        ...
        for mem in memory:
            H, W = mem.shape[-2:]
            mem = rearrange(mem, 'bt c h w -> bt (h w) c')
            # PWAM fusion
            # Compute Channel Attention
            z = mem.permute(0, 2, 1).mean(dim=2)
            fc_out_1 = self.relu(self.fc1(z))
            fc_out_2 = self.sigmoid(self.fc2(fc_out_1))
            ...
            op_align = self.object_position_align_blk(mem, H, W, static_feat, static_mask, motion_feat, motion_mask) #l b
            x_add = x_residul_gate + op_align
            ...
            mem_new = mem + x_add
            # print(src_new.shape) #5 448 256
            mem_new = rearrange(mem_new, 'bt (h w) c -> bt c h w', h=H)

            memorys_new.append(mem_new)
        ...
        memory_fusion = sum([F.interpolate(x, size=(tar_h, tar_w), mode="bicubic", align_corners=False) for x in memorys_new])  # bt, c, h, w
        ...
        #todo
        out["feauture_map"] = memorys_new[-1]
        # out["feauture_map"] = srcs[-2]
        # text_word_features_re = repeat(text_word_features, 'l b c -> b c l n', n=text_word_features.shape[0])
        # out["feauture_map"] = text_word_features_re[0]
        return out
    ...
    def proceesing_text_decoupling(self, text): # simple implementation
        ...
        motion_map = torch.zeros(num_tokens)
        static_map = torch.zeros(num_tokens)
