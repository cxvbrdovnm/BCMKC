...
class MultiModalSwinTransformer(nn.Module):
    def __init__(self,
        ...
        self.tmem = TMEM(dim=sum(num_features), num_blocks=num_tmem)
        self._freeze_stages()
    ...
    def forward(self, x, l, l_mask, t, t_mask, p, p_mask):
        ...
        # ablation studies
        # outs = self.cim(outs)  # RMSIN, Cross-scale Interaction Module
...
class MMBasicLayer(nn.Module):
    def __init__(self,
        ...
        self.context_pixel_attn = PWAM(dim,  # both the visual input and for combining, num of channels
                                       dim,  # v_in
                                       768,  # l_in  # 768 for bert feature, 512 for clip
                                       dim,  # key
                                       dim,  # value
    ...
    def forward(self, x, H, W, l, l_mask, t, t_mask, p, p_mask):
        """ Forward function.
...
class OPAB(nn.Module):
    ...
    # x: Fi imag feat  t: FG class feat  p: FS position feat
...
def get_captions(captions):
    ...
    for token in doc:
        ...
        # 完整句法的主语为主动或被动语句
        if "nsubj" in token.dep_ or "nsubjpass" in token.dep_:
            class_name = token.text
            break
    ...
    # tokenized_sentence = word_tokenize(captions)
    # grammar = r"""
    #                 PP: {<IN><DT>?<JJ.*>?<NN>}
    #                     {<IN><DT>?<JJ.*>?<JJ>}
    #                     {<IN><DT>?<JJ.*><VBD>}
...
if __name__ == "__main__":
    train_json_path = '../ReferFormer/data/ref-youtube-vos/meta_expressions/train/meta_expressions.json'
    ...
    all_exps = []
    for key in data:
        expressions = data[key].get("expressions", {})
        for expr_key in expressions:
            exp = expressions[expr_key].get("exp")
            if exp:
                # all_exps.append(exp)
                class_name = get_captions(exp)
                location = None
                ...
                # 写入 JSON 文件
                with open('train_spacy.json', 'w') as ff:
                    json.dump(output_data, ff, indent=4)
