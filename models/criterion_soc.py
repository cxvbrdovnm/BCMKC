...
class SetCriterion(nn.Module):
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        #print(src_boxes.shape, 'src_boxes') #1 5 5 4
        bs, nf, nq = src_boxes.shape[:3]
        src_boxes = src_boxes.transpose(1, 2)

        idx = self._get_src_permutation_idx(indices)
        #print(idx, 'idx') #tensor[0] tensor[4]
        src_boxes = src_boxes[idx]
        #print(src_boxes.shape, 'src_boxes_idx') # 1, 5, 4
        ...
        #print(loss_bbox) #1 5 4

        losses = {}
        losses['loss_bbox'] = loss_bbox.sum() / num_boxes
        #print(losses['loss_bbox']) #0.639
    ...
    def vl_loss(self, outputs, targets, indices, num_boxes):
        # print(outputs['voc_hs'].shape)
        # print(outputs['text_sentence_feature'].shape)
        device = outputs['voc_hs'].device
        output_logit = outputs['voc_hs']  # [B Nq C]
        output_logit = output_logit / torch.tensor(output_logit.shape[2])
        text_feature = outputs['text_sentence_feature']  # [b c]
        # [b C]
        bs = output_logit.shape[0]
        text_sentence_feature = text_feature.unsqueeze(1)  # [B 1 C]
        # print(text_sentence_feature.shape)
        query_text_sim = output_logit @ text_sentence_feature.transpose(1, 2)  # [B Nq 1] b q c * b c 1

        query_text_sim = (query_text_sim.squeeze(-1))  # [B, Nq]
        _, query_referred_indices = self._get_query_referred_indices(indices, targets, bs) #
        # print(query_referred_indices) #[1]

        # generate ground-truth
        gt = torch.zeros_like(query_text_sim) #b q
        # else:  # assume that the referred object is visible in every frame:
        # print([torch.arange(bs), query_referred_indices]) #0 1
        gt[torch.arange(bs), query_referred_indices] = torch.tensor([1.0], device=device, dtype=gt.dtype)
        # print(gt.shape) #torch.Size([1, 5])
        # print(gt) #tensor([[0., 1., 0., 0., 0.]], device='cuda:0')
        # matcher loss
        ...
        # print(loss.shape) #4 1 5
        #print([torch.arange(bs), query_referred_indices]) #[tensor([0]), tensor([1], device='cuda:0')]
    ...
    def _get_query_referred_indices(self, indices, targets, B):
        """
        ...
        for idx, ((query_idxs, target_idxs), target) in enumerate(zip(indices[:B], targets[:B])):
            #target: frames_idx, labels, boxes, masks, valid, orig_size, size, area
            #ref_query_idx = query_idxs[torch.where(target_idxs == target['referred_instance_idx'])[0]]
            ref_query_idx = query_idxs  # [1]
    ...
    def forward(self, outputs, targets):
        ...
        if 'aux_outputs' in outputs:
            ...
            #print(len(aux_indices)) #3
            #print(aux_indices)
            #[[(tensor([1], device='cuda:0'), tensor([0], device='cuda:0'))], [(tensor([4], device='cuda:0'), tensor([0], device='cuda:0'))], [(tensor([4], device='cuda:0'), tensor([0], device='cuda:0'))]]
