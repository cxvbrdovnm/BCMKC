...
from clip.transformer import VisionTransformer
...
class ClipOutput(dict):
    ...
    def save(self, idx: int, clip_feat: torch.Tensor):
        ...
        #print(self[idx].shape, self[f"{idx}_cls_token"].shape)
        #torch.Size([5, 768, 5, 7]) torch.Size([1, 5, 768])  10条
...
class FeatureExtractor(nn.Module):
    def __init__(
        ...
        self.image_size = visual_encoder.image_size #224 224
        self.patch_size = visual_encoder.patch_size #32 32
        self.grid_size = visual_encoder.grid_size #7 7
        self.num_features = visual_encoder.ln_pre.normalized_shape[0] #768

        # print(self.image_size, self.patch_size, self.grid_size, self.num_features)
        #todo
        #self.input_patchnorm = visual_encoder.input_patchnorm
        # self.patchnorm_pre_ln = visual_encoder.patchnorm_pre_ln
        self.input_patchnorm = False #变形计算
    ...
    def forward(self, x: torch.Tensor):
        if self.input_patchnorm:
            ...
            _, _, h, w = x.shape
            #print(x.shape, 'x') #5 768 5 7
            x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
            x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
            #print(x.shape, 'x permute') #5 35 768 bt hw ?
        ...
        #print(x.shape, '+class bed') #5 36 768
...
class RecWithAttnbiasHead(nn.Module):
    def __init__(
        ...
        self.global_average_pool = False #池化了
