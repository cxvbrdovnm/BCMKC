...
class VideoSwinTransformerBackbone(nn.Module):
    ...
    """
    def __init__(self, backbone_pretrained: bool, backbone_pretrained_path, train_backbone: bool, out_indices: tuple, **kwargs):
...
class Backbone(BackboneBase):
    ...
    def __init__(self, name: str,
        ...
        out_indices = tuple([i for i in range(output_levels)])  # control output level
        strides = [int(2**(i+2)) for i in out_indices]
        num_channels = [int(cfgs['embed_dim'] * 2**i) for i in out_indices]
        # print(strides, num_channels, 'aaaa')  # [4, 8, 16, 32] [96, 192, 384, 768] / [4, 8, 16] [96, 192, 384]
        backbone = VideoSwinTransformerBackbone(True, pretrained, True, out_indices, **cfgs)
...
def build_video_swin_backbone(args):
    ...
    backbone = Backbone(args.backbone, args.use_checkpoint, args.backbone_pretrained, args.output_levels)
...
if __name__ == '__main__':
    ...
    # model = VideoSwinTransformerBackbone(True, os.path.join(os.path.getcwd(), '../../pretrained_backbones/swin_tiny_patch244_window877_kinetics400_1k.pth'), True, **cfgs).cuda()
    model = VideoSwinTransformerBackbone(False, None, True, (0,1,2), **cfgs).cuda()
