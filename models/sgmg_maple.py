...
class SgMg(nn.Module):
    ...
    def __init__(self, args, backbone, transformer, maple,
        ...
        #todo init module
        self.maple = maple
        self.clip_text_ln = nn.Linear(512, 256)
        self.clip_img_ln = nn.Linear(512, 256)
        self.clip_text_fuse = nn.MultiheadAttention(256,8)
    ...
    def forward(self, samples: NestedTensor, captions, targets):
        ...
        #todo get maple feature
        maple_log_loss, maple_image_features, maple_text_features = self.maple(samples.tensors, text_sentence_features)
        #print(maple_log_loss, 'maple loss') #[5 1]'s average
        # print(maple_image_features.shape, 'maple image') #5 512
        # print(maple_text_features.shape, 'maple text') #89 512
        maple_image_features = self.clip_img_ln(maple_image_features) #5 256
        maple_text_features = self.clip_text_ln(maple_text_features) #89 256
        ...
        for l, (feat, pos_l) in enumerate(zip(features[-multi_scale_level:], visual_pos[-multi_scale_level:])):
            ...
            maple_image_features_re = maple_image_features.unsqueeze(2).unsqueeze(3).repeat(1, 1, h, w)
            src_proj_l = src_proj_l + maple_image_features_re
        ...
        text_embed = repeat(text_sentence_features, 'b c -> t b c', t=t)
        maple_text_features = maple_text_features.unsqueeze(1)
        text_embed = self.clip_text_fuse(text_embed, maple_text_features, maple_text_features)
        # print(text_embed[0].shape, len(text_embed))
        text_embed = repeat(text_embed[0], 't b c -> b t q c', q=self.num_queries)
