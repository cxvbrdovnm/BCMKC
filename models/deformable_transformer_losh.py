...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        encoder_layer = DeformableTransformerEncoderLayer(d_model, dim_feedforward,
                                                          ...
                                                          num_feature_levels,
                                                          nhead, enc_n_points)
        self.encoder = DeformableTransformerEncoder(encoder_layer, num_encoder_layers//2)
    ...
    def forward(self, srcs, tgt, masks, pos_embeds, query_embed=None):
        ...
        valid_ratios = torch.stack([self.get_valid_ratio(m) for m in masks], 1)  #non padding ratio on h w
        # encoder only encode image feature
        # memory: [bs*t, \sigma(hi*wi), c]
        #print(src_flatten.shape, lvl_pos_embed_flatten.shape, mask_flatten.shape) #38 1869 256, 38 1869 256, 38 1869
        ...
        if self.two_stage:
            ...
            # Use learnable queries to get reference points
            b, t, q, c = tgt.shape  # text
            tgt = rearrange(tgt, 'b t q c -> (b t) q c')
            query_embed = repeat(query_embed, 'o c -> b t o c', b=b, t=t).flatten(0,1)
...
#todo
class long_short_cross_attention(nn.Module):
    def __init__(self,
    ...
    def forward(self, src, pos, reference_points, spatial_shapes, level_start_index, padding_mask=None):
        ...
        long_pos = pos[:bt, :, :]
        short_pos = pos[bt:, :, :]
        #print(padding_mask.shape) # 2bt hw
        long_masks = padding_mask[:bt, :]
        short_masks = padding_mask[bt:, :]
        long_reference_points = reference_points[:bt]
        short_reference_points = reference_points[bt:]

        #Fl -> CA

        #print(short_pos.shape, long_masks.shape)
        with autocast(enabled=False):
            long_src2, long_sampling_locations, long_attention_weights = self.cross_attn(
                self.with_pos_embed(long_src, long_pos),
                long_reference_points,
                short_pos, spatial_shapes,
                level_start_index, long_masks)

        long_src = long_src + self.dropout1(long_src2)
        long_src = self.norm1(long_src)

        #Fs -> SA
        with autocast(enabled=False):
            short_src2, short_sampling_locations, short_attention_weights = self.self_attn(
                self.with_pos_embed(short_src, short_pos),
                short_reference_points,
                short_pos, spatial_shapes,
                level_start_index, short_masks)
        short_src = short_src + self.dropout4(short_src2)
        short_src = self.norm4(short_src)

        src = torch.cat((long_src, short_src), dim=0)
...
class DeformableTransformerEncoderLayer(nn.Module):
    def __init__(self,
        ...
        self.CA_SA_enc = long_short_cross_attention()
    ...
    def forward(self, src, pos, reference_points, spatial_shapes, level_start_index, padding_mask=None):
        ...
        #even block that incloud CA and SA
        src = self.CA_SA_enc(src, pos, reference_points, spatial_shapes, level_start_index, padding_mask)
...
class DeformableTransformerEncoder(nn.Module):
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None):
        output = src
        reference_points = self.get_reference_points(spatial_shapes, valid_ratios, device=src.device) # torch.Size([5, 5950, 4, 2])
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            # TODO: hyperpara 30.  Select topk from n_head * n_level * n_points corresponding points
            ...
            if self.bbox_embed is not None:
                ...
                if reference_points.shape[-1] == 4:
                    ...
                    new_reference_points[..., :2] = tmp[..., :2] + inverse_sigmoid(reference_points)  # Add linear predict motion offsets
                    new_reference_points = new_reference_points.sigmoid()
