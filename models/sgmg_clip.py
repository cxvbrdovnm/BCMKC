    ...
    def __init__(self, args, backbone, transformer, num_classes, num_queries, num_feature_levels,
        ...
        if num_feature_levels > 1:
            ...
            for idx_ in range(num_backbone_outs):
                ...
                in_channels = backbone_channels[-3:][idx_]
        ...
        self.mask_refine = MSO(mask_dim=self.dynamic_mask_channels, img_dim=backbone_channels[:2], out_dim=self.dynamic_mask_channels)
    ...
    def forward(self, samples: NestedTensor, captions, targets):
        ...
        if self.num_feature_levels > len(srcs):
            ...
            for l in range(_len_srcs, self.num_feature_levels):
                ...
                mask = features[-1].mask[:, :h, :w]
                # pos_l = visual_pos[-1]
                pos_l = torch.nn.functional.interpolate(
                    visual_pos[-1],
                    size=(h, w),
                    mode='bilinear',
                    align_corners=False
        ...
        # print(mask_features.shape, 'mask feat')
...
def build(args):
    ...
    from models.clip_as_backbone import build_clip_backbone
    backbone = build_clip_backbone(args)
