...
# 通过堆叠多个 ResidualAttentionBlock 模块来捕捉时间序列中的长距离依赖关系
...
class video_header(nn.Module):
    def __init__(self, vid_head, interaction, clip_state_dict):
        ...
        if self.vid_header == "Transf":
            ...
            # layer_num = 6
            layer_num = 2
            self.transformer = TemporalTransformer(width=embed_dim, layers=layer_num, heads=transformer_heads)
    ...
    def agg_video_feat(self, x):
        ...
        if self.vid_header == "None":
            ...
            position_ids = position_ids.unsqueeze(0).expand(x.size(0), -1)  # 1 t -> b t
            frame_position_embeddings = self.frame_position_embeddings(position_ids)  # (context_length, embed_dim)
    ...
    def get_logits(self, vid_emb, text_emb, cls_emb):
        vid_emb = self.agg_video_feat(vid_emb)  # b t c  NLD

        if self.interaction == 'DP':  # dot product
            # 视频特征池化
            ...
            ## Text-to-Video: Video Concept Spotting
        elif self.interaction == 'VCS':  # video concept spotting
            # 归一化
            cls_emb = cls_emb / cls_emb.norm(dim=-1, keepdim=True)
            vid_emb = vid_emb / vid_emb.norm(dim=-1, keepdim=True)
            text_emb = text_emb / text_emb.norm(dim=-1, keepdim=True)
            # 文本特征和视频特征之间的相似度矩阵 每个文本序列与每个视频序列在每个时间步上的相似度
            sims = torch.einsum('awd,btd->abwt', [text_emb, vid_emb])  # b_t b_v l_t l_v
            # 对相似度矩阵进行softmax归一化，得到视频特征的注意力权重。dim=-1 表示在视频的时间维度上进行归一化。
            att_weight_v = F.softmax(sims / 0.01, dim=-1)  # abwt
            # 对文本的时间维度进行平均，得到注意力权重
            att_weight_v = att_weight_v.mean(dim=-2)  # abt  b_t b_v l_v

            # 对视频特征进行加权求和，得到经过注意力加权的视频特征
            v_att = torch.einsum('abt,btd->abd', [att_weight_v, vid_emb])  # b_t b_v c

            # new 点积，得到最终的相似度矩阵
            t2v_logits = torch.einsum('abd,ad->ab', [v_att, cls_emb])  # b_t b_v

            # 表示视频特征和类别嵌入之间的相似度矩阵
            logit = t2v_logits.transpose(1, 0)

        return v_att, logit#, vid_emb
...
class sentence_text_logit(nn.Module):
    ...
    def get_sentence_query_logits(self, query_cls_feat, sentece_cls_feat, query_mask=None, sentence_mask=None):
        query_cls_feat = self.query_fc(query_cls_feat)  # b_q c 89 512
        sentece_cls_feat = self.sentence_fc(sentece_cls_feat)  # b_s c
        # 归一化, 使它们的范数为 1
        ...
        # 相似度矩阵
        logit = sentece_cls_feat @ query_cls_feat.t()  # b_s b_q
        return logit
