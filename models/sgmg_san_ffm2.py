...
class SgMg(nn.Module):
    ...
    def forward(self, samples: NestedTensor, captions, targets):
        ...
        for l, (feat, pos_l) in enumerate(zip(features[-multi_scale_level:], visual_pos[-multi_scale_level:])):
            ...
            # todo fuse feature
            src_proj_l = rearrange(src_proj_l, 'bt c h w -> bt (h w) c')
            # print(src.shape, 'src') c:256 512 1024
            src_clip = self.fuse(l, src_proj_l, clip_features, image=True)  # bt L c
            # print(src_clip.shape, 'src_clip') #5 596 256
            src_proj_l = rearrange(src_clip, 'bt (h w) c -> bt c h w', h=h)
        ...
        if self.num_feature_levels > len(srcs):
            ...
            for l in range(_len_srcs, self.num_feature_levels):
                ...
                src = rearrange(src_clip, 'bt (h w) c -> bt c h w', h=h)
