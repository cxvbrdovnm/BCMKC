...
from .segmentation import VisionLanguageFusionModule, LanguageVisionFusionModule, LanguageVisionFusionModule_new, ASPP
...
from .mmca import MultiModalCausalAttention, SimpleMMCA


import warnings
warnings.filterwarnings("ignore")

from .san import san_bike, prepare_query, fuse
...
class SgMg(nn.Module):
    ...
    def __init__(self, args, backbone, transformer, num_classes, num_queries, num_feature_levels,
        ...
        # todo init module
        self.l2v = LanguageVisionFusionModule_new(d_model=hidden_dim, nhead=8)
        # self.l2v = MultiModalCausalAttention(256)
        # self.l2v = SimpleMMCA(256, 8)
        # self.aspp = ASPP(256, 256)
        #available models = ['RN50', 'RN50-quickgelu', 'RN101', 'RN101-quickgelu', 'RN50x4', 'RN50x16', 'RN50x64', 'ViT-B-32', 'ViT-B-32-quickgelu', 'ViT-B-16', 'ViT-L-14', 'ViT-L-14-336']
        self.san_bike = san_bike("ViT-B-16", args)
        self.fuse = fuse(hidden_dim=256)

        # clip_text_linear = nn.Sequential(
        #     nn.Linear(1024, 512),
        #     nn.LayerNorm(512),
        #     nn.Linear(512, 256),
        #     nn.LayerNorm(256)
        # )
        #
        # self.clip_text_linear = nn.ModuleList(clip_text_linear)
        # self.clip_query = nn.MultiheadAttention(hidden_dim, 8, dropout=0.0)
    ...
    def forward(self, samples: NestedTensor, captions, targets):
        ...
        # # print(text_word_masks.shape) #1 9 b l
        # text_word_features = text_word_features.permute(1, 0, 2)  # b l c

        # todo get clip features
        # with torch.no_grad():
        #     clip_features = self.get_clip_features(samples)
        # clip_features = self.get_clip_features(samples)
            # print(clip_features[0].shape) #5 768 5 7
        # print(len(clip_features)) #20 most 50
            # 20 一共10层残差网络，【0-9】为bt c h w， 【0-9_cls_token】为1 bt c
            #print(clip_features["0_cls_token"])
        clip_image_features, clip_visual_text, clip_cls_new = self.san_bike(samples, captions)
        ...
        for l, (feat, pos_l) in enumerate(zip(features[-multi_scale_level:], visual_pos[-multi_scale_level:])):
            ...
            #print(src.shape, 'src') c:256 512 1024
            src_clip = self.fuse(l, src, clip_image_features, clip_cls_new, image=True)  # bt L c
            # print(src_clip.shape, 'src_clip') #5 596 256

            # text_word_clip = self.fuse(l, text_word_features, clip_features, image=False)
            #print(text_word_clip.shape, 'text_word_clip') #bt l c

            src_clip = rearrange(src_clip, 'bt (h w) c -> bt c h w', h=h)
            src_proj_l = self.input_proj[l](src_clip)
            ...
            src_proj_l = self.fusion_module(visual=src_proj_l,
                                            #text=text_word_clip.permute(1, 0, 2), #l b c
                                            text=text_word_features.permute(1, 0, 2), #l b c
            ...
            # src_proj_l = self.aspp(src_proj_l)
            ...
            # todo vision guide text
            # #src_l = rearrange(src_proj_l, '(b t) c h w -> (t h w) b c', b=b)
            # src_l = src_proj_l
            # mask_l = rearrange(mask, '(b t) h w -> b (t h w)', t=t, b=b)
            # pos_lang = rearrange(pos_l, "(b t) c h w -> (t h w) b c", t=t, b=b)
            # lan_l = self.l2v(#visual=text_l,
            #                             text=text_word_features.permute(1, 0, 2),
            #                            visual=src_l,
            #                            #visual_key_padding_mask=mask_l,
            #                            #visual_pos=pos_lang,
            #                            text_pos=None
            #                            )
            # lan_l = rearrange(lan_l, 'l b c -> b l c')

            # src_l = rearrange(src_proj_l, 'bt c h w -> bt (h w) c')
            # _, lan_l = self.l2v(src_l, text_word_features) #b l c -> b l c

            # mmca
            src_l = rearrange(src_proj_l, 'bt c h w -> bt c (h w)')
            ss_map = F.softmax(src_l, dim=2)  # 生成了一个软注意力图，表示每个空间位置在特征通道上的相对重要性
            visual_proto = torch.bmm(ss_map, src_l.transpose(1, 2))  # bt c' c

            # text_map = F.softmax(text_word_features.transpose(1, 2), dim=2)
            # text_proto = torch.bmm(text_map, text_word_features.transpose(1, 2))
            # _, lan_l = self.l2v(visual_proto, text_word_features) #b l c -> b l c

            # simple
            text_map = F.softmax(text_word_features.transpose(1, 2), dim=2) # b l c -> b c l
            text_proto = torch.bmm(text_map, text_word_features) #b c' c
            lan_l = self.l2v(visual_proto, text_proto)
        ...
        if self.num_feature_levels > len(srcs):
            ...
            for l in range(_len_srcs, self.num_feature_levels):
                if l == _len_srcs:
                    ...
                    src = rearrange(features[-1].tensors, 'bt c h w -> bt (h w) c')
                    src_clip = self.fuse(l, src, clip_image_features, clip_cls_new, image=True)  # bt L c
                    src_clip = rearrange(src_clip, 'bt (h w) c -> bt c h w', h=h)

                    src = self.input_proj[l](src_clip)
                else:
                    n, c, h, w = srcs[-1].shape
                    src = rearrange(srcs[-1], 'bt c h w -> bt (h w) c')
                    src_clip = self.fuse(l, src, clip_image_features, clip_cls_new, image=True)  # bt L c
                    src_clip = rearrange(src_clip, 'bt (h w) c -> bt c h w', h=h)
        ...
        # text_embed = repeat(text_features_new, 'b c -> t b c', t=t)
        # clip_text = rearrange(clip_features["9_cls_token"].squeeze(0), '(b t) c -> t b c', t=t) #5 1 1024
        # #clip_text = self.clip_text_linear(clip_text)
        # for layer in self.clip_text_linear:
        #     clip_text = layer(clip_text)
        # text_embed_mix = self.clip_query(text_embed, clip_text, clip_text)[0]
        # text_embed = text_embed * text_embed_mix
        # text_embed = repeat(text_embed, 't b c -> b t q c', q=self.num_queries)

        query_new = self.query_norm(text_features_new + text_sentence_features_new)

        text_embed = repeat(query_new, 'b c -> b t q c', t=t, q=self.num_queries)

        #text_embed, query_embeds = self.add_query_token(text_sentence_features, query_embeds) #b t q+5 c, q+5 c
        ...
        #print(hs.shape, 'hs') #4 6 10 256

        # hs = self.hs_linear(hs.permute(0, 1, 3, 2)).permute(0, 1, 3, 2)
        # init_reference = self.reference_linear(init_reference.permute(0, 2, 1)).permute(0, 2, 1)
        # inter_references = self.references_linear(inter_references.permute(0, 1, 3, 2)).permute(0, 1, 3, 2)

        # hs = hs[:, :, :5, :]
        # init_reference = init_reference[:, :5, :]
        # inter_references = inter_references[:, :, :5, :]
