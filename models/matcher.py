...
class HungarianMatcher(nn.Module):
    ...
    def __init__(self, cost_class: float = 1, cost_bbox: float = 1, cost_giou: float = 1,
                       cost_mask: float = 1, cost_dice: float = 1, cost_boundary: float = 1, num_classes: int = 1):
        """Creates the matcher
        ...
        self.mask_out_stride = 2

    @torch.no_grad()
    def forward(self, outputs, targets, b):
        ...
        for i in range(b):
            ...
            cost_class = torch.stack(cost_class, dim=0).mean(0)
            ...
            #         ).squeeze(0)
            #
            # print(out_mask.transpose(0, 1).shape) #5 5 112 96
            # print(tgt_mask.unsqueeze(0).shape) #1 5 112 96
...
def build_matcher(args):
    ...
    return HungarianMatcher(cost_class=args.set_cost_class,
                            ...
                            cost_boundary=args.set_cost_boundary,
