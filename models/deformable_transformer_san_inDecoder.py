...
from .san_inDecoder import get_clip_features, prepare_srcs, fuse
...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        # todo init_clip
        self.prepare_srcs = prepare_srcs(token_num_queries=5, hidden_dim=256)
    ...
    def forward(self, srcs, tgt, masks, pos_embeds, clip_features, query_embed=None):
        ...
        if self.two_stage:
            ...
            # # todo
            # tgt_add_query, query_embed_add_query = self.prepare_srcs(tgt, query_embed)
            reference_points = self.reference_points(query_embed).sigmoid()

            init_reference_out = reference_points

        # decoder
        hs, inter_references, inter_samples = self.decoder(clip_features, tgt, reference_points, memory,
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def forward(self, clip_features, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            # todo add clip feature
            src_clip = self.fuse(lid, src, clip_features)  # bt L c
            ...
            # todo src -> src_clip (memory)
            output, sampling_locations, attention_weights = layer(output, query_pos, reference_points_input,
                                                        src_clip, src_spatial_shapes, src_level_start_index, src_padding_mask)
