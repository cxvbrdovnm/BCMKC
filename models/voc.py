...
class VOC(nn.Module):
    ...
    def __init__(
        ...
        self.num_layers = args.voc_dec_layers
        ...
        self.fq_pos = nn.Embedding(self.num_frame_queries, args.mask_dim)

        #### learnable query p.e.
        self.query_embed = nn.Embedding(self.num_queries, args.mask_dim)
        # self.query_feat = nn.Embedding(self.num_queries, config['input_dim'])

        self.decoder_norm = nn.LayerNorm(args.mask_dim)

        self.enc_layers = args.voc_dec_layers
    ...
    def forward(self, frame_query, language_query):
        ...
        output = rearrange(output, 'b l nq c -> nq (l b) c') #5 4 256
...
if __name__ == '__main__':
    ...
    frame_query = torch.randn(size=(4, 5, 1, 5, 256))
    lanaguae_query = torch.randn(size=(1, 256))
    output = voc(frame_query, lanaguae_query)
    print(output.size())
