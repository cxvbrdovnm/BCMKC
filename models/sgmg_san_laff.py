...
from .LAFF import Multi_head_MyApply_FusionAttention
...
class SgMg(nn.Module):
    ...
    def __init__(self, args, backbone, transformer, num_classes, num_queries, num_feature_levels,
        ...
        # todo init module
        # self.l2v = LanguageVisionFusionModule(d_model=hidden_dim, nhead=8)
        ...
        self.laff = Multi_head_MyApply_FusionAttention(embed_dim=256, multi_heads=8, dim_per_head=256//8)
        self.laff_linear = nn.Linear(256*4, 256)
    ...
    def forward(self, samples: NestedTensor, captions, targets):
        ...
        for l, (feat, pos_l) in enumerate(zip(features[-multi_scale_level:], visual_pos[-multi_scale_level:])):
            ...
            src_l = rearrange(src_proj_l, '(b t) c h w -> b (t h w) c', t=t)
            #lang_l = self.laff(src_l, text_sentence_features)
            lang_l = self.laff(text_word_features, src_l)
            # print(lang_l.shape, 'lang_l') #1 8 32
            langs.append(lang_l.reshape(b, -1)) #b c
            # print(lang_l)
            ...
            #                             visual=text_word_features.permute(1, 0, 2),
            #                            text=src_l,
            #                            text_key_padding_mask=mask_l,
            #                            text_pos=pos_lang,
            #                            visual_pos=None
            #                            )
            #
        ...
        if self.num_feature_levels > len(srcs):
            ...
            for l in range(_len_srcs, self.num_feature_levels):
                ...
                src_l = rearrange(src, '(b t) c h w -> b (t h w) c', t=t)
                lang_l = self.laff(src_l, text_sentence_features)
                # print(lang_l.shape, 'lang_l') #1 8 32
                langs.append(lang_l.reshape(b, -1))  # b c
        ...
        langs = torch.stack(langs, dim=1).reshape(b, -1)
        text_features_new = self.laff_linear(langs)
        # print(text_features_new)
