...
from .criterion_soc import SetCriterion
...
class SgMg(nn.Module):
    ...
    def __init__(self, args, backbone, transformer, voc,
        ...
        self.lvf = LanguageVisionFusionModule(d_model=hidden_dim, nhead=8)

        self.vl_loss = True
        # if self.vl_loss:
        #     self.is_referred_head = nn.Linear(d_model, 2)  # binary 'is referred?' prediction
    ...
    def forward(self, samples: NestedTensor, captions, targets):
        ...
        for l, (feat, pos_l) in enumerate(zip(features[-multi_scale_level:], visual_pos[-multi_scale_level:])):
            ...
            lan_l = self.lvf(#visual=text_l,
                                        visual=text_word_features, #l b c
                                       text=src_l,
        ...
        #todo
        text_embed = torch.zeros_like(query_embeds)
        text_embed = repeat(text_embed, 'q c -> b t q c', b=b, t=t)
        ...
        voc_hs = self.voc(hs, text_sentence_features)  # VOC_hs [L B NQ C]

        # residual
        frame = hs.shape[1]
        hs_voc = repeat(voc_hs, 'l b n c -> l t b n c', t=frame)
        hs_voc = hs + hs_voc
        hs = rearrange(hs_voc, 'l t b n c -> l (b t) n c')
        ...
        if self.training:
            ...
            with torch.no_grad():
                ...
                if self.aux_loss:
                    #text_features_repeat = repeat(text_features, 'b c -> l b c', l=4)
                    #out['aux_outputs'] = self._set_aux_loss(outputs_class, outputs_coord, outputs_seg_masks_formatcher, voc_hs, text_features_repeat)
            ...
            if self.aux_loss:
                ...
                #text_features_repeat = repeat(text_features, 'b c -> l b c', l=4)
                #out['aux_outputs'] = self._set_aux_loss_comprehensive(outputs_class, outputs_coord, outputs_seg_masks, outputs_seg_masks_formatcher, voc_hs, text_features_repeat)
                out['aux_outputs'] = self._set_aux_loss_comprehensive(outputs_class, outputs_coord, outputs_seg_masks,
                                                                      outputs_seg_masks_formatcher)
        ...
        # todo
        # out['voc_hs'] = voc_hs[-1, :, :, :]  # VOC_hs [B NQ C]
        # out['text_sentence_feature'] = text_features
        #print(out['voc_hs'].shape)
    ...
    def _set_aux_loss_comprehensive(self, outputs_class, outputs_coord, outputs_seg_masks, outputs_seg_masks_low):
        ...
        return [{"pred_logits": a, "pred_boxes": b, "pred_masks": c, "pred_masks_low": d}
                for a, b, c, d in
                zip(outputs_class[:-1], outputs_coord[:-1], outputs_seg_masks[:-1], outputs_seg_masks_low[:-1])]
