...
from .rpe_transformer import build_transformer
...
class SgMg(nn.Module):
    ...
    def __init__(self, args, backbone, transformer,
        ...
        #todo rpe init
        self.rpe_transformer = rpe_transformer
        self.query_embed_rpe = nn.Embedding(num_queries, hidden_dim)
    ...
    def forward(self, samples: NestedTensor, captions, targets):
        ...
        for l, (feat, pos_l) in enumerate(zip(features[-multi_scale_level:], visual_pos[-multi_scale_level:])):
            ...
            # todo prepare u_srcs and relative_poses, use 3 levels
            # prepare srcs, u_srcs, u_poses
            r_src = rearrange(src_proj_l, '(b t) c h w -> b t c h w', t=t)
            re_pos = rearrange(pos_l, '(b t) c h w -> b t c h w', t=t)
            re_mask = rearrange(mask, '(b t) h w -> b t h w', t=t)

            # for each level features, get each frame's former features
            former_srcs = []
            former_poses = []
            former_masks = []
            for j in range(t):
                # for first frame, relative info is none, as self-attention
                if j < self.time_gap:
                    former_srcs.append(r_src[:, 0, :, :, :])
                    former_poses.append(re_pos[:, 0, :, :, :])
                    former_masks.append(re_mask[:, 0, :, :])

                # for 1 2 3 4 5, use former frame to get relative info
                else:
                    former_srcs.append(r_src[:, (j - self.time_gap), :, :, :])  # b c h w
                    former_poses.append(re_pos[:, (j - self.time_gap), :, :, :])
                    former_masks.append(re_mask[:, (j - self.time_gap), :, :])

            u_srcs.append(rearrange(torch.stack(former_srcs), 't b c h w -> b t c h w'))  # l b t c h w
            u_poses.append(rearrange(torch.stack(former_poses), 't b c h w -> b t c h w'))
            u_masks.append(rearrange(torch.stack(former_masks), 't b h w -> b t h w'))
        ...
        hs_rpe = [[]]
        memory_rpe = []
        # o_srcs = rearrange(srcs[2], '(b t) c h w -> b t c h w', b=b)
        for i in range(t):
            t_u_srcs = []
            t_u_pos = []
            t_u_masks = []
            for l in range(len(u_srcs)):
                t_u_srcs.append(u_srcs[l][:, i, :, :, :])
                t_u_pos.append(u_poses[l][:, i, :, :, :])
                t_u_masks.append(u_masks[l][:, i, :, :])

            # get st_info for each frame, b c h w => q b c
            # relative mulit-head attention, for every frame
            query_embed_rpe = self.query_embed_rpe.weight  # q c
            hs_t, memory_t = self.rpe_transformer(t_u_srcs[-1], hs_rpe[-1], t_u_masks[-1], query_embed_rpe, t_u_pos[-1])
            hs_rpe.append(hs_t)
            memory_rpe.append(memory_t)

        hs_rpe = rearrange(torch.stack(hs_rpe[-t:]), 't q b c -> (b t) q c')

        memory_rpe = rearrange(torch.stack(memory_rpe), 't b c h w -> (b t) c h w')
        ...
        for lvl in range(hs.shape[0]):
            ...
            outputs_class = self.class_embed[lvl](hs[lvl] + hs_rpe)
            tmp = self.bbox_embed[lvl](hs[lvl] + hs_rpe)
        ...
        for lvl in range(hs.shape[0]):
            dynamic_mask_head_params = self.controller(hs[lvl] + hs_rpe)   # [batch_size*time, num_queries_per_frame, num_params]
...
def build(args):
    ...
    rpe_transformer = build_transformer(args)
