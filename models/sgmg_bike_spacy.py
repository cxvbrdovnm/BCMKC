...
from .fianet import get_captions
...
class SgMg(nn.Module):
    ...
    def __init__(self, args, backbone, transformer,
        ...
        self.bike_clip, self.video_head = bike(args)
        # b-32: 512, l-14: 768
        clip_dim = 512
        self.clip_visual_linear = nn.Linear(clip_dim, 256)
        # self.clip_visual_text_linear = nn.Linear(clip_dim, 256)
        # self.tame = TMEM(dim=hidden_dim*4, num_blocks=2, channels=[256, 256, 256, 256])
        self.clip_visual_linear = nn.Linear(clip_dim, 256)
        self.clip_visual_text_linear = nn.Linear(clip_dim, 256)
    ...
    def forward(self, samples: NestedTensor, captions, targets):
        ...
        # print(all_class)
        # class_name = get_captions(captions)
        # print(type(class_name)) # str
        class_name = text_prompt(captions).to(images_resized.device)
        # print(class_name)

        image_embedding, cls_embedding, text_embedding, logit_scale = \
            self.bike_clip(images_resized, class_name, return_token=True)

        # print(image_embedding.shape, 'img emb')  #5 512 0.5
        # print(image_embedding)
        # print(cls_embedding.shape, 'cls emb')  #1 512 0.1
        # print(cls_embedding)
        # print(text_embedding.shape, 'txt emb')  #1 77 512 0.2
        # print(text_embedding)
        # print(logit_scale) #100.

        image_embedding_sv = image_embedding.clone().view(b_o, t_o, -1)
        images_fusion, S_V, img_feat = self.video_head(image_embedding_sv, text_embedding, cls_embedding)
        #logits = logit_scale * S_V
        # print(S_V.shape, 'S_V') #1 1
        # print(S_V) #0.21
        # print(images_fusion.shape, "image fusion") #1 1 512 0.01
        # print(images_fusion)  #0.03, 应该乘logit_scale
        # print(img_feat.shape, "img_feat") #1 5 512 0.05
        # print(img_feat)
        # print(images_fusion.shape)

        # clip_visual = self.clip_visual_linear(images_fusion[0]).repeat(t, 1)  # bt c 512->256
        clip_visual = self.clip_visual_linear(image_embedding.float())  # bt c 512->256
        clip_visual_text = self.clip_visual_text_linear(images_fusion[0]).repeat(t, 1)  # bt c 512->256
        clip_visual_add = clip_visual + clip_visual_text

        # clip_visual = self.clip_visual_linear(image_embedding)
        # #视频与哪个类别最相关的索引
        ...
        clip_cls_new = self.clip_cls_linear(cls_embedding).unsqueeze(0) #1 b 256
        ...
        # lang = text_word_features.permute(1, 2, 0).repeat(t, 1, 1) # l b c -> bt c l
        # l_mask = text_word_masks.repeat(t, 1, 1).permute(0, 2, 1) #bt l 1
        # # print(lang.shape)
        # # print(l_mask.shape)
        # srcs = self.tame(srcs, lang, l_mask)
        '''
        inputs: 
        srcs -> [bt c h w] * 4
        lang -> [bt c l]
