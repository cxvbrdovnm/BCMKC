...
class Attention_1(nn.Module):
    """
        ...
        Args: - local_embs: local embeddings, shape: (batch_size, L, embed_dim)
              - raw_global_emb: tagert embbeding, shape: (batch_size, L, embed_dim)
    ...
    def forward(self, local_embs: torch.Tensor, raw_global_emb=None):
        ...
        g_emb = F.interpolate(raw_global_emb.permute(0, 2, 1).unsqueeze(3), size=(local_embs.size(1), 1),
                              mode='bilinear', align_corners=False).squeeze(3).permute(0, 2, 1)
        ...
        if self.mul:
            ...
            #todo
            #b l c
            # print(raw_global_emb.shape) #[1, 2240, 32]
            #print(local_embs.shape) #[1, 12, 32]

            # print(g_emb.shape) #[1, 12, 32]
            common = local_embs.mul(g_emb)  # (b, L, emb_size)
        ...
        if self.with_ave:
            ...
            new_global = new_global_weights * new_global + raw_global_weight * g_emb
    ...
    def forward(self, local_embs, raw_global_emb=None, attn_mask=None):
        ...
        if self.output_type == 'cls_embedding':
            cls_embedding = self.cls_embedding(
                torch.zeros((batch_size), dtype=torch.long, device=local_embs.device)).unsqueeze(1)
    ...
    def forward(self, local_embs, raw_global_emb=None, attn_mask=None):
        ...
        if self.split_head:
            ...
            if raw_global_emb is not None:
                raw_global_emb = raw_global_emb.view(batch_size, -1, num_heads, dim_per_head)
        else:
            # repeat by heads, (batch_size, L, embed_dim)-> (batch_size, heads, L, embed_dim)
            local_embs = local_embs.unsqueeze(2).repeat(1, 1, num_heads, 1)
            if raw_global_emb is not None:
                raw_global_emb = raw_global_emb.unsqueeze(2).repeat(1, 1, num_heads, 1)

        # print(raw_global_emb.shape, 'test') #[1, 2240, 8, 32]
        global_emb_list = []
        for i in range(self.multi_heads):
            if raw_global_emb is not None:
                global_emb_list.append(
                    self.attention_layer[i](local_embs[:, :, i, :], raw_global_emb=raw_global_emb[:, :, i, :]))
            else:
                global_emb_list.append(self.attention_layer[i](local_embs[:, :, i, :]))
...
class Multi_head_MyApply_FusionAttention(Multi_head_My_AttentionMulGlobal):
    """
