...
from .deformable_transformer_losh import build_deforamble_transformer
from .segmentation import VisionLanguageFusionModule
from .matcher import build_matcher

from .criterion_losh import SetCriterion
...
class SgMg(nn.Module):
    ...
    def __init__(self, args, backbone, transformer, num_classes, num_queries, num_feature_levels,
        ...
        self.tokenizer = RobertaTokenizerFast.from_pretrained('../ReferFormer/pretrained_text_model/roberta_base/')
        self.text_encoder = RobertaModel.from_pretrained('../ReferFormer/pretrained_text_model/roberta_base/')
    ...
    def build_controller(self):
        ...
        #todo
        #self.is_referred_head = nn.Linear(self.hidden_dim, 2)
        # binary 'is referred?' prediction head for object queries
    ...
    def forward(self, samples: NestedTensor, captions, targets):
        ...
        # todo
        # #你需要获取该设备的名称或编号
        # device = next(self.backbone.parameters()).device  # 获取backbone参数的第一个设备
        # # 然后将samples移动到该设备
        ...
        # print("text_pos.shape", text_pos.shape)#12 1 256
        # print("text_word_features.shape", text_word_features.shape)#12 1 256
        # print("text_word_masks.shape", text_word_masks.shape)#1 12
        # for dictionary in targets:
        #     print(dictionary.keys()) #['frames_idx', 'labels', 'boxes', 'masks', 'valid', 'orig_size', 'size', 'area']

        text_queries = captions
        #short_text_queries = [t["labels"] for t in targets]

        #spacy.require_gpu()
        spacy.prefer_gpu()
        nlp = spacy.load("en_core_web_md")
        doc = nlp(str(text_queries))
        ...
        if first_verb is not None:
            short_text_queries = " ".join([token.text for token in doc[2:first_verb.i+1]])
            short_text_queries = [short_text_queries]
        else:
            short_text_queries = text_queries


        # print(text_queries, 'long text')
        # print(short_text_queries, 'short text')

        # ===================================for long text queries===================================
        txt_memory, txt_pad_mask = self.losh_forward_text(list(text_queries), device=visual_pos[0].device)
        # add temporal dim to txt memory & padding mask:
        #txt_memory = repeat(txt_memory, 'l b c -> l (t b) c', t=t)
        #txt_pad_mask = repeat(txt_pad_mask, 'b l -> (t b) l', t=t)
        ...
        short_txt_memory, short_txt_pad_mask = self.losh_forward_text(list(short_text_queries), device=visual_pos[0].device)
        #short_txt_memory = repeat(short_txt_memory, 'l b c -> l (t b) c', t=t)
        #short_txt_pad_mask = repeat(short_txt_pad_mask, 'b l -> (t b) l', t=t)
        # print('short_txt_pad:', short_txt_pad_mask)

        # pad the short memory and padding mask
        len_short_query, b, c = short_txt_memory.shape
        diff_long_short = len_long_query - len_short_query
        if diff_long_short > 0:
            diff_txt_memory = torch.zeros([diff_long_short, b, c], dtype=short_txt_memory.dtype, device=visual_pos[0].device)
            diff_pad_mask = torch.ones([b,diff_long_short], dtype=short_txt_pad_mask.dtype, device=visual_pos[0].device)
        ...
        # print("txt_memory.shape: ",txt_memory.shape) #12 1 256
        # print("short_txt_memory.shape: ", short_txt_memory.shape) #12 25 256
        # #print(diff_txt_memory.shape, diff_txt_memory.dtype)
        # print("txt_pad_mask.shape: ", txt_pad_mask.shape) #1 12
        # print("short_txt_pad_mask.shape: ", short_txt_pad_mask.shape) #1 12
        # #print(diff_pad_mask.shape, diff_pad_mask.dtype)
        # #print(diff_pad_mask)
        # print("short_txt_memory.shape: ", short_txt_memory.shape) #12 1 256
        # print("short_txt_pad_mask.shape: ", short_txt_pad_mask.shape) #1 12
        # ===================================for short text queries===================================
        ...
        poses = []
        multi_scale_level = 3
        high_filter_long = None
        high_filter_short = None
        for l, (feat, pos_l) in enumerate(zip(features[-multi_scale_level:], visual_pos[-multi_scale_level:])):
            ...
            src_proj_l_long, high_filter_long = self.input_fft[l](src_proj_l, high_filter_long)
            src_proj_l_long = rearrange(src_proj_l_long, '(b t) c h w -> t h w b c', b=b, t=t)
            src_proj_l_long = self.fusion_module(visual=src_proj_l_long,
                                                 text=txt_memory,
                                                 text_key_padding_mask=txt_pad_mask,
                                                 text_pos=torch.zeros_like(txt_memory),
                                                 visual_pos=None
                                                 )
            src_proj_l_long = rearrange(src_proj_l_long, '(t h w) b c -> (b t) c h w', t=t, h=h, w=w)
            src_proj_l_long, high_filter_long = self.input_fft_post[l](src_proj_l_long, high_filter_long)

            src_proj_l_short, high_filter_short = self.input_fft[l](src_proj_l, high_filter_short)
            src_proj_l_short = rearrange(src_proj_l_short, '(b t) c h w -> t h w b c', b=b, t=t)
            src_proj_l_short = self.fusion_module(visual=src_proj_l_short,
                                                 text=short_txt_memory,
                                                 text_key_padding_mask=short_txt_pad_mask,
                                                 text_pos=torch.zeros_like(short_txt_memory),
                                                 visual_pos=None
                                                 )
            src_proj_l_short = rearrange(src_proj_l_short, '(t h w) b c -> (b t) c h w', t=t, h=h, w=w)
            src_proj_l_short, high_filter_short = self.input_fft_post[l](src_proj_l_short, high_filter_short)

            srcs_long.append(src_proj_l_long)
            srcs_short.append(src_proj_l_short)
            masks.append(mask)
            poses.append(pos_l)
            assert mask is not None

        if self.num_feature_levels > len(srcs_long):
            _len_srcs = self.num_feature_levels - 1
            for l in range(_len_srcs, self.num_feature_levels):
                if l == _len_srcs:
                    src_long = self.input_proj[l](features[-1].tensors)
                    src_short = src_long.clone()
                else:
                    src_long = self.input_proj[l](srcs_long[-1])
                    src_short = self.input_proj[l](srcs_short[-1])
                m = samples.mask
                mask = F.interpolate(m[None].float(), size=src_long.shape[-2:]).to(torch.bool)[0]
                pos_l = self.backbone[1](NestedTensor(src_long, mask)).to(src_long.dtype)
                n, c, h, w = src_long.shape

                src_long, high_filter_long = self.input_fft[l](src_long, high_filter_long)
                src_long = rearrange(src_long, '(b t) c h w -> t h w b c', b=b, t=t)
                src_long = self.fusion_module(visual=src_long,
                                              text=txt_memory,
                                              text_key_padding_mask=txt_pad_mask,
                                              text_pos=torch.zeros_like(txt_memory),
                                              visual_pos=None
                                              )
                src_long = rearrange(src_long, '(t h w) b c -> (b t) c h w', t=t, h=h, w=w)
                src_long, high_filter_long = self.input_fft_post[l](src_long, high_filter_long)

                src_short, high_filter_short = self.input_fft[l](src_short, high_filter_short)
                src_short = rearrange(src_short, '(b t) c h w -> t h w b c', b=b, t=t)
                src_short = self.fusion_module(visual=src_short,
                                              text=short_txt_memory,
                                              text_key_padding_mask=short_txt_pad_mask,
                                              text_pos=torch.zeros_like(short_txt_memory),
                                              visual_pos=None
                                              )
                src_short = rearrange(src_short, '(t h w) b c -> (b t) c h w', t=t, h=h, w=w)
                src_short, high_filter_short = self.input_fft_post[l](src_short, high_filter_short)

                srcs_long.append(src_long)
                srcs_short.append(src_short)
                masks.append(mask)
                poses.append(pos_l)
        ...
        seq_masks = []
        poses_embed = []
        for l in range(len(srcs_long)):
            encoder_src_seq = torch.cat((srcs_long[l], srcs_short[l]), dim=0)
            seq_mask = torch.cat((masks[l], masks[l]), dim=0)
            pos_embed = torch.cat((poses[l], poses[l]), dim=0)
            # print(encoder_src_seq.shape) # 2bt c h w
            # print(seq_mask.shape) # 2bt h w
            # print(pos_embed.shape) # 2bt c h w
            encoder_srcs_seq.append(encoder_src_seq)
            seq_masks.append(seq_mask)
            poses_embed.append(pos_embed)
        # ==========get encoder_src_seq_long and short_encoder_scr_seq, then concatenate them together==========
        ...
        #todo
        text_embed = torch.cat((text_embed, text_embed), dim=0) #create same query for long and short

        hs, memory, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, inter_samples = \
                                            self.transformer(encoder_srcs_seq, text_embed, seq_masks, poses_embed, query_embeds)
        ...
        outputs_class = rearrange(outputs_class, 'l (b t) q k -> l b t q k', b=2*b, t=t)
        outputs_coord = rearrange(outputs_coord, 'l (b t) q n -> l b t q n', b=2*b, t=t)
        out['pred_logits'] = outputs_class[-1]
        out['pred_boxes'] = outputs_coord[-1]

        long_out['pred_logits'] = out['pred_logits'][:b, :, :]
        short_out['pred_logits'] = out['pred_logits'][b:, :, :]
        long_out['pred_boxes'] = out['pred_boxes'][:b, :, :, :]
        short_out['pred_boxes'] = out['pred_boxes'][b:, :, :, :]

        # Segmentation
        #todo CPK x memory
        tar_h, tar_w = memory[0].shape[-2:] # 16 28
        memory_fusion = sum([F.interpolate(x, size=(tar_h, tar_w), mode="bicubic", align_corners=False) for x in memory])  # 2bt, c, h, w
        mask_features = rearrange(memory_fusion, '(b t) c h w -> b t c h w', b=2*b, t=t) # 2 5 256 16 28
        outputs_seg_masks = []
        ...
        for lvl in range(hs.shape[0]):
            ...
            dynamic_mask_head_params = rearrange(dynamic_mask_head_params, '(b t) q n -> b (t q) n', b=2*b, t=t) # 2 25 4416
            lvl_references = inter_references[lvl, ..., :2]
            lvl_references = rearrange(lvl_references, '(b t) q n -> b (t q) n', b=2*b, t=t) # 2 25 2
            outputs_seg_mask = self.dynamic_mask_with_coords(mask_features, dynamic_mask_head_params, lvl_references, targets, b)
            outputs_seg_masks.append(outputs_seg_mask)
            # use pixel_shuffle to convert [h1,w1,p^2] to [h1p,w1p]
            outputs_seg_masks_formatcher.append(rearrange(F.pixel_shuffle(outputs_seg_mask.flatten(0,1), 4).squeeze(1),
                                                          '(b t q) h w -> b t q h w', b=2*b,t=t,q=self.num_queries))
        ...
        if self.training:
            ...
            with torch.no_grad():
                ...
                # print(outputs_seg_masks_formatcher[-1].shape) #2 5 5 64 112
                indices = self.matcher(out, targets, b)
                out["main_matcher_index"] = indices
                select_idxs = _get_src_permutation_idx(indices)

                long_out['pred_masks'] = out["pred_masks"][:b, :, :, :, :] #b t q h w
                short_out['pred_masks'] = out["pred_masks"][b:, :, :, :, :]
                long_out['main_matcher_index'] = out["main_matcher_index"]
                short_out['main_matcher_index'] = out["main_matcher_index"]

                #print(short_out['pred_masks'].shape) #1 5 5 64 112
                ...
                if self.aux_loss:
                    ...
                    for i, aux_outputs in enumerate(out['aux_outputs']):
                        aux_indices.append(self.matcher(aux_outputs, targets, b))
                    out["aux_matcher_index"] = aux_indices
                    aux_select_idxs = [_get_src_permutation_idx(x) for x in aux_indices]


                    long_outputs_seg_masks_formatcher = [x[:b, :, :, :, :] for x in outputs_seg_masks_formatcher]
                    long_out['aux_outputs'] = self._set_aux_loss(
                        outputs_class[:, :b, :, :, :],  # l b t q k
                        outputs_coord[:, :b, :, :, :],  # l b t q n
                        long_outputs_seg_masks_formatcher)  # l b t q h w
                    short_outputs_seg_masks_formatcher = [x[b:, :, :, :, :] for x in outputs_seg_masks_formatcher]
                    short_out['aux_outputs'] = self._set_aux_loss(
                        outputs_class[:, b:, :, :, :],  # l b t q k
                        outputs_coord[:, b:, :, :, :],  # l b t q n
                        short_outputs_seg_masks_formatcher)  # l b t q h w
                    long_out['aux_matcher_index'] = out["aux_matcher_index"]
                    short_out['aux_matcher_index'] = out["aux_matcher_index"]

                    # print(len(out['aux_outputs'])) #3
                    # print(len(long_out['aux_outputs'])) #3
                    # print(len(short_out['aux_outputs'])) #3

            if self.aux_loss:
                select_idxs = aux_select_idxs + [select_idxs]
                #print(outputs_seg_masks[0].shape, 'outputs_seg_masks') #2, 25, 16, 16, 28
                long_pred_masks = [rearrange(x[:b], 'b (t q) c h w -> b q t c h w', t=t)[y] for x, y in
                                   zip(outputs_seg_masks, select_idxs)]  # b t c h w
                #print(pred_masks[0].shape, 'pred_masks') #1, 5, 16, 16, 28 zhelichuwenti
                long_pred_masks = [x.flatten(0, 1) for x in long_pred_masks]
                long_pred_masks = [self.mask_refine(x, features[:2]) for x in long_pred_masks]
                long_pred_masks = [F.pixel_shuffle(x, 4) for x in long_pred_masks]
                long_outputs_seg_masks = [rearrange(x.squeeze(1), '(b t) h w -> b t h w', b=b, t=t) for x in long_pred_masks]
                long_outputs_seg_masks_formatcher = [x[:b].transpose(1, 2)[y] for x, y in
                                                zip(outputs_seg_masks_formatcher, select_idxs)]  # b t h w



                short_pred_masks = [rearrange(x[b:], 'b (t q) c h w -> b q t c h w', t=t)[y] for x, y in
                                   zip(outputs_seg_masks, select_idxs)]  # b t c h w
                # print(pred_masks[0].shape, 'pred_masks') #1, 5, 16, 16, 28 zhelichuwenti
                short_pred_masks = [x.flatten(0, 1) for x in short_pred_masks]
                short_pred_masks = [self.mask_refine(x, features[:2]) for x in short_pred_masks]
                short_pred_masks = [F.pixel_shuffle(x, 4) for x in short_pred_masks]
                short_outputs_seg_masks = [rearrange(x.squeeze(1), '(b t) h w -> b t h w', b=b, t=t) for x in
                                          short_pred_masks]
                short_outputs_seg_masks_formatcher = [x[b:].transpose(1, 2)[y] for x, y in
                                                     zip(outputs_seg_masks_formatcher, select_idxs)]  # b t h w
                ...
                long_out['pred_masks'] = long_outputs_seg_masks[-1]
                long_out['pred_masks_low'] = long_outputs_seg_masks_formatcher[-1]
                long_out['aux_outputs'] = self._set_aux_loss_comprehensive(
                    outputs_class[:, :b, :, :, :],  # l b t q k
                    outputs_coord[:, :b, :, :, :],  # l b t q n
                    long_outputs_seg_masks,  #l b t h w
                    long_outputs_seg_masks_formatcher) #l b t h w

                short_out['pred_masks'] = short_outputs_seg_masks[-1]
                short_out['pred_masks_low'] = short_outputs_seg_masks_formatcher[-1]
                short_out['aux_outputs'] = self._set_aux_loss_comprehensive(
                    outputs_class[:, b:, :, :, :],  # l b t q k
                    outputs_coord[:, b:, :, :, :],  # l b t q n
                    short_outputs_seg_masks,  #l b t h w
                    short_outputs_seg_masks_formatcher)  #l b t h w

                # print(long_out["pred_masks"].shape) # 1 5 128 224
                # print(short_out['pred_masks'].shape) #1 5 128 224
            ...
            long_out['pred_logits'] = out['pred_logits'][:b, :, :]
            long_out['pred_boxes'] = out['pred_boxes'][:b, :, :, :]
            long_out['reference_points'] = out['reference_points'][:b, :, :, :]
            short_out['pred_logits'] = out['pred_logits'][b:, :, :]
            short_out['pred_boxes'] = out['pred_boxes'][b:, :, :, :]
            short_out['reference_points'] = out['reference_points'][b:, :, :, :]

            #todo MSO
            #print(pred_masks.shape) #38 16 28 50
            long_pred_masks = self.mask_refine(pred_masks[:(b*t)], features[:2])
            long_pred_masks = F.pixel_shuffle(long_pred_masks, 4)
            long_outputs_seg_masks = rearrange(long_pred_masks.squeeze(1), '(b t) h w -> b t h w', b=b, t=t)
            long_out["pred_masks"] = long_outputs_seg_masks.unsqueeze(2)

            short_pred_masks = self.mask_refine(pred_masks[(b*t):], features[:2])
            short_pred_masks = F.pixel_shuffle(short_pred_masks, 4)
            short_outputs_seg_masks = rearrange(short_pred_masks.squeeze(1), '(b t) h w -> b t h w', b=b, t=t)
            short_out["pred_masks"] = short_outputs_seg_masks.unsqueeze(2)
    ...
    def losh_forward_text(self, text_queries, device):
        ...
        txt_memory = self.text_proj(txt_memory)  # change text embeddings dim to model dim
        # Invert attention mask that we get from huggingface because its the opposite in pytorch transformer
        txt_pad_mask = tokenized_queries.attention_mask.ne(1).bool()  # [B, S]
        return txt_memory, txt_pad_mask


    def dynamic_mask_with_coords(self, mask_features, mask_head_params, reference_points, targets, b):
        """
        ...
        bb, t, c, h, w = mask_features.shape
        ...
        if self.rel_coord:
            ...
            relative_coords = reference_points.reshape(bb, t, q, 1, 1, 2) - \
...
def build(args):
    ...
    if args.masks:
        ...
        #todo
        weight_dict['loss_conditioned_iou'] = args.conditioned_iou_loss_coef
