...
def main(args):
    ...
    # root = Path(args.ytvos_demo_path)
...
def sub_processor(lock, pid, args, data, save_path_prefix, save_visualize_path_prefix, img_folder, video_list, result_dict):
    ...
    for idx_, video in enumerate(video_list):
        ...
        #multi-object visualize
        #保存每个exp画好的t帧
        img_multi_object_i = []
        # mask_color = [(0,255,255)]
        mask_color = [(0, 128, 0)]
        exp_color = [(255, 0, 0), (0,255,0),(128, 0, 128)] 
        ...
        for i in range(num_expressions):
            ...
            with torch.no_grad():
                with autocast(args.amp):
                    #todo
                    #[outputs, _] = model([imgs], [exp], [target])
            ...
            if args.visualize:
                for t, frame in enumerate(frames):
                    ...
                    #save box
                    box_location.append(draw_boxes[0])

                    # draw boxes
                    xmin, ymin, xmax, ymax = draw_boxes[0]
                    draw.rectangle(((xmin, ymin), (xmax, ymax)), outline=tuple(mask_color[0]),
                                   width=2)
                    ...
                    draw_reference_points(draw, ref_points, source_img.size, color=mask_color[0])

                    # draw mask
                    source_img = vis_add_mask(source_img, all_pred_masks[t], mask_color[0])
                    ...
                    if i==0:
                        ...
                        if isinstance(img_multi_object_i[t], np.ndarray):
                            img_multi_object_i[t] = Image.fromarray(img_multi_object_i[t])
                        else:
                            #print(f"Type of img_multi_object_i[t] is {type(img_multi_object_i[t])}, expected numpy.ndarray")
                            pass
                            
                        draw_i = ImageDraw.Draw(img_multi_object_i[t])
                        draw_i.rectangle(((xmin, ymin), (xmax, ymax)), outline=tuple(mask_color[0]),
                                   width=2)
                        draw_reference_points(draw_i, ref_points, source_img.size, mask_color[0])
                        img_multi_object_i[t] = vis_add_mask(img_multi_object_i[t], all_pred_masks[t], mask_color[0])
                

                
                #save as video
                # 目标文件夹路径
                folder_path = save_visualize_path_dir
                # 输出视频路径
                output_video_path = save_visualize_path_dir + 'output_video.mp4'
                # 帧率
                fps = 3
                # 视频宽度和高度
                frame_width = origin_w
                frame_height = origin_h
                # 指定颜色（BGR格式）
                text_color = tuple(exp_color[i % len(exp_color)]) #RGB->BGR
                ...
                png_files.sort()  # 确保文件按顺序排列

                # 创建视频写入对象
                fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # 视频编码
                video_result = cv2.VideoWriter(output_video_path, fourcc, fps, (frame_width, frame_height))

                if not video_result.isOpened():
                    print("Error: Could not open video writer.")
                else:
                    # 读取每张图片并添加文本
                    for t, image_file in  enumerate(png_files):
                        image_path = os.path.join(folder_path, image_file)
                        img = cv2.imread(image_path)
                        if img is None:
                            print(f"Error: Could not read image {image_path}.")
                            continue
                        x = int(box_location[t][0])
                        y = int(box_location[t][1])
                        text_position = (x + 10, y - 10)  # 在边界框上方10像素处
                        # 在图片上添加实验信息
                        cv2.putText(img, draw_exp, text_position, font, 1, text_color, 2, cv2.LINE_AA)
                        ...
                        #更新multi_object_i
                        if i==0:
                            img_multi_object_i.append(img)
                        else:
                            img_multi = np.array(img_multi_object_i[t])
                            cv2.putText(img_multi, draw_exp, text_position, font, 1, text_color, 2, cv2.LINE_AA)
                            img_multi_object_i[t] = img_multi
        ...
        #画mulit-object的video
        multi_save_path = os.path.join(save_visualize_path_prefix, video)
        # 输出视频路径
        multi_output_video_path = multi_save_path + '.mp4'

        # 创建视频写入对象
        multi_video_result = cv2.VideoWriter(multi_output_video_path, fourcc, fps, (frame_width, frame_height))
        for img in img_multi_object_i:
            if isinstance(img, np.ndarray):
                pass
            else:
                img = np.array(img)
            multi_video_result.write(img)
        multi_video_result.release()
