...
def random_nested_tensor(batch_size: int, num_frames: int, channels: int, height: int, width: int):
    # 随机生成 tensors [num_frames x channels x height x width]
    tensors = torch.rand(batch_size, num_frames, channels, height, width)

    # 随机生成 mask [num_frames x height x width]，值为 0 或 1
    mask = torch.randint(0, 2, (batch_size, num_frames, height, width))

    return NestedTensor(tensors.to('cuda'), mask.to('cuda'))


def random_captions(num_captions: int, max_length: int = 10):
    # 随机生成一些字符串作为 captions
    import string
    import random
    ...
    for _ in range(num_captions):
        ...
        caption = ''.join(random.choices(string.ascii_letters + ' ', k=length))
        captions.append(caption)

    return captions


def random_targets(num_targets: int, num_boxes: int):
    # 随机生成一些字典作为 targets
    targets = []
    for _ in range(num_targets):
        boxes = torch.rand(num_boxes, 4)  # 随机生成一些框的坐标
        labels = torch.randint(1, 10, (num_boxes,))  # 随机生成一些标签
        sizes = torch.tensor([360, 640])
        target = {
            'boxes': boxes.to('cuda'),
...
def main(args):
    ...
    height = 112    # 图像高度
    width = 200     # 图像宽度
    num_captions = 1  # caption 数量
    num_targets = 1   # target 数量
    num_boxes = 1     # 每个 target 中的框数量

    # # 随机生成输入
    # samples = random_nested_tensor(batch_size, num_frames, channels, height, width)
    # captions = random_captions(num_captions)
    # targets = random_targets(num_targets, num_boxes)
    #
    # flops, params = profile(model, inputs=(samples, captions, targets))
